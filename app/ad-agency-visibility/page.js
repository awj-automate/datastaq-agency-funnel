...
// Single-purpose paid-traffic funnel: land from a Meta/Google ad, watch the
// VSL, book a call. Booking lives on the existing /schedule page (this repo
// has no Calendly embed/modal — every CTA routes there, same as app/page.js).
...
export default function AdAgencyVisibilityPage() {
  const [openFaq, setOpenFaq] = useState(null);

  // Fire ViewContent once when the funnel is viewed (same pattern as
  // app/schedule/page.js). PageView is already tracked globally by
  // MetaPageView + the GA4 config in app/layout.js.
  useEffect(() => {
    trackViewContent({ content_name: 'ad_agency_visibility' });
  }, []);

  // Reveal-on-scroll — identical IntersectionObserver pattern to app/page.js.
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* ============ 9. FINAL CTA ============ */}
      {/* Repo has no Calendly embed/modal — booking is the dedicated /schedule
          page, where Schedule + Purchase conversion events already fire. */}
      ...
      <footer>
        <div className="footer-inner">
          ...
          {/* TODO: confirm /privacy and /terms routes exist, or update hrefs. */}
      ...
      {/*
        ============================================================
        /ad-agency-visibility — CONFIG NOTES
        ============================================================
        ONLY ENV VAR NEEDED
        ...
        - OG + Twitter images: generated by opengraph-image.js / twitter-image.js
          in this folder. metadataBase (layout.js) resolves automatically from
          Vercel's VERCEL_PROJECT_PRODUCTION_URL — nothing to set.
        ...
        BOOKING
        - This repo has no Calendly embed/modal. Every CTA routes to the
          existing /schedule page (same as app/page.js). Booking-completion
          conversion events (Schedule + Purchase) already fire there.
        ...
        SEO
        - noindex,nofollow + OG/Twitter tags set in ./layout.js via the
          Next.js metadata API (consistent with the root app/layout.js).
        ============================================================
      */}
    </>
  );
